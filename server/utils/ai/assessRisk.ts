...
export async function assessResumeRisk(
  ...
  // Частота смен — для контекста (считает код, не LLM).
  const tenureBlock = opts.tenure.hasStructuredDates
    ? `Частота смен мест работы: ${opts.tenure.jobHoppingLevel} (${opts.tenure.jobsCount} мест, средний срок ${opts.tenure.avgTenureMonths} мес).`
    : 'Данные о частоте смен отсутствуют.'
  ...
  const system
    = `Ты — аналитик отбора в ATS. Сегодня ${iso}. `
      + 'Работай ТОЛЬКО с фактами из сырого текста резюме. '
      + 'Анализируй СОДЕРЖАНИЕ резюме: '
