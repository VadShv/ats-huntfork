...
const VOLATILE_TOP_LEVEL_KEYS = new Set([
  ...
  // Служебный маркер Huntfork: содержит structuredAt/documentId/provider/model —
  // меняется при каждом прогоне ИИ, но не отражает правок кандидата. Без этого
  // исключения одинаковые по содержанию резюме давали бы разный хеш при каждом
  // структурировании → «unchanged»/дебаунс недостижимы, плодились лишние версии.
  '_hf',
])
