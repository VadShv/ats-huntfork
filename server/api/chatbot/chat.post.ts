...
const BASE_SYSTEM_PROMPT = [
  ...
  '- Use `list_jobs` / `search_candidates` to discover IDs, then drill down with `get_*` tools.',
  '- `search_candidates` returns `{ total, returned, truncated, candidates, hint }`. If `truncated: true`, ALWAYS state both numbers ("нашёл N кандидатов, показываю первые M") and offer to narrow filters or paginate. Never silently drop matches.',
  '- COST-AWARE TOOL USE: `read_resume` is expensive (loads up to 12k chars of resume text). For shortlists and rankings, prefer the cheap path: `search_candidates` → `get_candidate` (already includes applications + scores) → `get_application_scores` (for the "why"). Call `read_resume` only when the user explicitly asks for resume details, a specific quote, or a deep dive into one candidate. NEVER call `read_resume` for every candidate in a top-N list — pick the 2–3 most relevant and stop.',
