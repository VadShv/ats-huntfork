...
// ── Промпты ─────────────────────────────────────────────────────────
const BASE_SYSTEM
  = 'Ты ассистент рекрутёра в ATS Huntfork. Отвечай на русском языке, кратко и по делу, '
  ...
  switch (mode) {
    ...
    case 'fit':
      return `${BASE_SYSTEM}\nСделай краткую выжимку профиля (до 120 слов), затем блок "### Соответствие вакансии": `
        ...
        + 'пробелы и риски, 2-3 вопроса для первого контакта. Это предварительная оценка для рекрутёра, не финальное решение.'
    case 'fragment':
      return `${BASE_SYSTEM}\nСделай краткую выжимку выделенного фрагмента: суть, ключевые факты, на что обратить внимание рекрутёру. До 150 слов.`
    case 'questions':
    ...
    case 'custom':
      return `${BASE_SYSTEM}\nИнструкция рекрутёра: ${instruction}`
    default:
      return BASE_SYSTEM
  }
}
