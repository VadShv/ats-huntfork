...
async function applyCandidate(candidateId: string) {
  ...
  try {
    ...
    applyError.value = err.data?.statusMessage ?? t('components.applyCandidateModal.failedToApply')
  } finally {
      ...
      <div class="relative bg-white dark:bg-surface-900 rounded-xl shadow-xl w-full max-w-md mx-4 max-h-[80vh] flex flex-col">
        ...
        <div class="px-5 pt-4">
          <div class="relative">
            ...
            <input
              v-model="searchInput"
              type="text"
              :placeholder="$t('components.applyCandidateModal.searchPlaceholder')"
        ...
        <div class="flex-1 overflow-y-auto px-5 py-3">
          <div v-if="searchStatus === 'pending'" class="text-center py-6 text-surface-400 text-sm">
            {{ $t('components.applyCandidateModal.searching') }}
          </div>

          <div v-else-if="candidates.length === 0" class="text-center py-6 text-surface-400 text-sm">
            {{ debouncedSearch ? $t('components.applyCandidateModal.noCandidatesFound') : $t('components.applyCandidateModal.noCandidatesInOrg') }}
          </div>
          ...
          <div v-else class="space-y-1">
            ...
            >
              ...
              <span class="text-xs text-brand-600 dark:text-brand-400 font-medium shrink-0 ml-2">
                {{ $t('components.applyCandidateModal.apply') }}
              </span>
            </button>
          </div>
        </div>
