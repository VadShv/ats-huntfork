 ...
 * Показывает зафиксированные данные на момент прикрепления (аудит версии).
 */
import { computed } from 'vue'
import { Bot, ShieldAlert, AlertTriangle } from 'lucide-vue-next'
...
<template>
  ...
  >
    <div class="flex items-center gap-1.5 text-xs font-semibold text-surface-600 dark:text-surface-300">
      ...
      {{ isScreening ? t('discussion_widgets.screening') : t('discussion_widgets.risk') }}
      <span class="ml-auto text-[10px] font-normal text-surface-400">
        {{ t('comment_snapshot.pinned') }}
      </span>
    </div>
