...
useSeoMeta({
  title: 'Лента',
  description: 'Полная лента событий вашей организации',
})
...
const filters = [
  { key: undefined, label: 'Все события', icon: History },
  { key: 'job', label: 'Вакансии', icon: Briefcase },
  { key: 'candidate', label: 'Кандидаты', icon: Users },
  { key: 'application', label: 'Отклики', icon: FileText },
...
function getActionStyle(action: string, resourceType: string, metadata?: Record<string, unknown> | null): ActionStyle {
  ...
  if (action === 'status_changed' && metadata) {
    ...
    return { icon: ArrowRight, ...colors, label: 'Перемещено' }
  }

  // Спринт 15.1: смена этапа воронки — единый стиль
...
function getEventDescription(item: TimelineItem): string {
  ...
  switch (item.action) {
    case 'created': return `${type}: создано`
    case 'updated': return `${type}: обновлено`
    case 'deleted': return `${type}: удалено`
    case 'status_changed': return `${type}: перемещено`
    case 'stage_changed': return `${type}: этап изменён`
    case 'comment_added': return `Комментарий к объекту: ${type.toLowerCase()}`
    case 'member_invited': return 'Участник приглашён'
    case 'member_removed': return 'Участник удалён'
    case 'member_role_changed': return 'Роль изменена'
    case 'scored': return `ИИ оценил: ${type}`
    case 'scheduled': return `${type}: запланировано`
      ...
      <div class="mt-3 relative">
        <div class="relative">
          ...
          <input
            v-model="searchQuery"
            type="text"
            placeholder="Поиск по имени, дате или ключевому слову…"
    ...
    >
      <div class="rounded-2xl border border-surface-200 dark:border-surface-800 bg-white dark:bg-surface-900 p-10 text-center max-w-sm">
        ...
        <p class="text-sm text-surface-500 dark:text-surface-400 leading-relaxed">
          События появятся здесь, когда вы создадите вакансию, добавите кандидата или обработаете отклик.
        </p>
        <NuxtLink
                ...
                <div v-if="section.directItems.length" class="divide-y divide-surface-100 dark:divide-surface-800/60">
                  ...
                  >
                    ...
                    <div class="flex items-center gap-2 shrink-0">
                      ...
                      <div v-else-if="item.actorId === null" class="flex items-center gap-1">
                        <div class="size-4 rounded-full bg-brand-100 dark:bg-brand-950 flex items-center justify-center">
                          <span class="text-[8px] font-bold text-brand-600 dark:text-brand-400">A</span>
                        </div>
                        <span class="text-[11px] text-surface-400 dark:text-surface-500">Система</span>
                      </div>
                  ...
                  <div class="divide-y divide-surface-50 dark:divide-surface-800/40">
                    ...
                    >
                      ...
                      <div class="flex items-center gap-2 shrink-0">
                        ...
                        <div v-else-if="item.actorId === null" class="flex items-center gap-1">
                          <div class="size-3.5 rounded-full bg-brand-100 dark:bg-brand-950 flex items-center justify-center">
                            <span class="text-[7px] font-bold text-brand-600 dark:text-brand-400">A</span>
                          </div>
                          <span class="text-[10px] text-surface-400 dark:text-surface-500">Система</span>
                        </div>
