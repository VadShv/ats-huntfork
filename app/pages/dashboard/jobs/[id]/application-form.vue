...
useSeoMeta({
  title: computed(() =>
    job.value ? `${t('dashboard.jobs.appForm.pageTitle')} — ${job.value.title} — Reqcore` : `${t('dashboard.jobs.appForm.pageTitle')} — Reqcore`,
  ),
})
...
async function saveRequirements() {
  ...
  try {
    ...
    requirementsError.value = err?.data?.statusMessage ?? t('dashboard.jobs.appForm.failedToSaveRequirements')
  } finally {
...
async function handleCreateLink() {
  ...
  try {
    ...
    toast.error(err?.data?.statusMessage ?? t('dashboard.jobs.appForm.failedToCreateLink'))
  } finally {
...
async function handleDeleteLink() {
  ...
  try {
    ...
    toast.error(err?.data?.statusMessage ?? t('dashboard.jobs.appForm.failedToDelete'))
  } finally {
...
<template>
  <div class="mx-auto max-w-3xl">
    ...
    <div v-if="fetchStatus === 'pending'" class="text-center py-12 text-surface-400">
      {{ t('dashboard.jobs.appForm.loading') }}
    </div>
    ...
    >
      {{ error.statusCode === 404 ? t('dashboard.jobs.appForm.jobNotFound') : t('dashboard.jobs.appForm.failedToLoad') }}
      <NuxtLink :to="$localePath('/dashboard')" class="underline ml-1">{{ t('dashboard.jobs.appForm.backToJobs') }}</NuxtLink>
    </div>

    <template v-else-if="job">
      <!-- Header -->
      <div class="mb-6">
        <h1 class="text-2xl font-bold text-surface-900 dark:text-surface-50">{{ t('dashboard.jobs.appForm.pageTitle') }}</h1>
        <p class="text-sm text-surface-500 dark:text-surface-400 mt-1">
          {{ t('dashboard.jobs.appForm.pageDesc') }} <strong>{{ job.title }}</strong>.
        </p>
      </div>
      ...
      <div v-if="job.status === 'open'" class="rounded-lg border border-brand-200 dark:border-brand-800 bg-brand-50 dark:bg-brand-950 p-5 mb-6">
        <div class="flex items-center gap-2 mb-2">
          ...
          <h2 class="text-sm font-semibold text-brand-700 dark:text-brand-300">{{ t('dashboard.jobs.appForm.appLinkTitle') }}</h2>
        </div>
        <p class="text-xs text-surface-600 dark:text-surface-400 mb-3">
          {{ t('dashboard.jobs.appForm.appLinkDesc') }}
        </p>
        <div class="flex items-center gap-2">
          ...
          >
            ...
            {{ linkCopied ? t('dashboard.jobs.appForm.copied') : t('dashboard.jobs.appForm.copy') }}
          </button>
        </div>
      </div>
      ...
      <div v-else class="rounded-lg border border-surface-200 dark:border-surface-800 bg-surface-50 dark:bg-surface-900 p-4 mb-6 text-sm text-surface-500 dark:text-surface-400">
        {{ t('dashboard.jobs.appForm.appLinkNotPublished') }}
      </div>

      <!-- Application Requirements -->
      <div class="rounded-lg border border-surface-200 dark:border-surface-800 bg-white dark:bg-surface-900 p-5 mb-6">
        <h2 class="text-sm font-semibold text-surface-700 dark:text-surface-300 mb-1">{{ t('dashboard.jobs.appForm.requirementsTitle') }}</h2>
        <p class="text-xs text-surface-400 dark:text-surface-500 mb-4">
          {{ t('dashboard.jobs.appForm.requirementsDesc') }}
        </p>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
          ...
          >
            ...
            <div>
              <span class="block text-sm font-medium text-surface-900 dark:text-surface-100">{{ t('dashboard.jobs.appForm.requireResume') }}</span>
              <span class="text-xs text-surface-500">{{ t('dashboard.jobs.appForm.requireResumeDesc') }}</span>
            </div>
          </button>
          <button
            type="button"
          ...
          >
            ...
            <div>
              <span class="block text-sm font-medium text-surface-900 dark:text-surface-100">{{ t('dashboard.jobs.appForm.requireCoverLetter') }}</span>
              <span class="text-xs text-surface-500">{{ t('dashboard.jobs.appForm.requireCoverLetterDesc') }}</span>
            </div>
          </button>
        </div>
        <button
        ...
        >
          {{ requirementsSaved ? t('dashboard.jobs.appForm.saved') : isSavingRequirements ? t('dashboard.jobs.appForm.saving') : t('dashboard.jobs.appForm.saveRequirements') }}
        </button>
      ...
      <div class="rounded-lg border border-surface-200 dark:border-surface-800 bg-white dark:bg-surface-900 p-5 mb-6">
        <div class="flex items-center justify-between mb-1">
          <div class="flex items-center gap-2">
            ...
            >
              {{ t('dashboard.jobs.appForm.trackingLinksTitle') }}
            </NuxtLink>
          </div>
          <button
            v-if="canManageLinks"
          ...
          >
            <Plus class="size-3.5" />
            {{ t('dashboard.jobs.appForm.newLink') }}
          </button>
        </div>
        <p class="text-xs text-surface-400 dark:text-surface-500 mb-4">
          {{ t('dashboard.jobs.appForm.trackingLinksDesc') }}
        </p>
        ...
        <div v-else-if="trackingLinks.length === 0" class="py-6 text-center">
          ...
          <p class="text-sm text-surface-400 dark:text-surface-500">{{ t('dashboard.jobs.appForm.noTrackingLinks') }}</p>
          <p class="text-xs text-surface-300 dark:text-surface-600 mt-1">{{ t('dashboard.jobs.appForm.noTrackingLinksHint') }}</p>
        </div>
            ...
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-2 mb-0.5">
                ...
                >
                  ...
                  {{ link.isActive ? t('dashboard.jobs.appForm.active') : t('dashboard.jobs.appForm.inactive') }}
                </span>
              </div>
              <div class="text-[11px] text-surface-400 dark:text-surface-500 tabular-nums">
                {{ link.clickCount }} {{ t('dashboard.jobs.appForm.clicks') }} · {{ link.applicationCount }} {{ t('dashboard.jobs.appForm.applications') }}
              </div>
            </div>
...
:title="t('dashboard.jobs.appForm.copyTrackingUrl')"
...
:title="link.isActive ? t('dashboard.jobs.appForm.deactivate') : t('dashboard.jobs.appForm.activate')"
      ...
      <div class="rounded-lg border border-surface-200 dark:border-surface-800 bg-white dark:bg-surface-900 p-5">
        <div class="flex items-center gap-2 mb-3">
          ...
          <h2 class="text-sm font-semibold text-surface-700 dark:text-surface-300">{{ t('dashboard.jobs.appForm.customQuestionsTitle') }}</h2>
        </div>
        <p class="text-xs text-surface-400 dark:text-surface-500 mb-4">
          {{ t('dashboard.jobs.appForm.customQuestionsDesc') }}
        </p>
      ...
      <div v-if="showCreateLinkModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-lg rounded-2xl border border-surface-200 dark:border-surface-800 bg-white dark:bg-surface-900 shadow-2xl">
          <div class="flex items-center justify-between px-6 py-4 border-b border-surface-100 dark:border-surface-800">
            <h2 class="text-base font-semibold text-surface-900 dark:text-surface-100">{{ t('dashboard.jobs.appForm.createLinkTitle') }}</h2>
            <button
          ...
          <form class="px-6 py-5 space-y-4" @submit.prevent="handleCreateLink">
            <div>
              <label for="link-name" class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-1.5">{{ t('dashboard.jobs.appForm.linkName') }}</label>
              <input
                id="link-name"
            ...
            <div>
              <label for="link-channel" class="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-1.5">{{ t('dashboard.jobs.appForm.sourceChannel') }}</label>
              <select
                id="link-channel"
              ...
              >
                <optgroup :label="t('dashboard.jobs.appForm.jobBoards')">
            ...
            <details class="group">
              <summary class="flex items-center gap-2 text-sm font-medium text-surface-500 dark:text-surface-400 cursor-pointer select-none hover:text-surface-700 dark:hover:text-surface-200 transition-colors">
                ...
                {{ t('dashboard.jobs.appForm.utmParams') }}
              </summary>
            ...
            <div class="flex items-center justify-end gap-3 pt-2">
              <button
                ...
                @click="showCreateLinkModal = false"
              >
                {{ t('dashboard.jobs.appForm.cancel') }}
              </button>
              <button
                type="submit"
              ...
              >
                {{ isCreatingLink ? t('dashboard.jobs.appForm.creating') : t('dashboard.jobs.appForm.createLink') }}
              </button>
            </div>
          </form>
        </div>
    ...
    <Teleport to="body">
      <div v-if="showDeleteLinkConfirm" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-sm rounded-2xl border border-surface-200 dark:border-surface-800 bg-white dark:bg-surface-900 shadow-2xl p-6 text-center">
          ...
          <h3 class="text-base font-semibold text-surface-900 dark:text-surface-100 mb-2">{{ t('dashboard.jobs.appForm.deleteLinkTitle') }}</h3>
          <p class="text-sm text-surface-500 dark:text-surface-400 mb-6">
            {{ t('dashboard.jobs.appForm.deleteLinkDesc') }}
          </p>
          <div class="flex items-center justify-center gap-3">
            ...
            >
              {{ t('dashboard.jobs.appForm.deleteLink') }}
            </button>
          </div>
        </div>
      </div>
