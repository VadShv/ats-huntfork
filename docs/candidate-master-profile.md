...
**Rule-based структуризаторы резюме (реализовано, сентябрь 2026):**
- `server/utils/ai/hh-text-structurer.ts`: два детерминированных профиля —
  `structureHhResumeText` (экспорт hh.ru: «Опыт работы —N лет») и
  `structurePortfolioResumeText` (кастомные «портфолио»-PDF: «ПРОФЕССИОНАЛЬНЫЙ
  ОПЫТ», даты после должности). Диспетчер `structureResumeRuleBased` пробует оба,
  null → LLM-fallback. `structureResumeFromText` вызывает диспетчер первым.
- Извлекает: ФИО, контакты, желаемую должность, ДР/пол, опыт (компания/должность/
  период/обязанности), образование (полный и краткий формат «высшее без вуза»).
- Проверено на реальных резюме: hh (Чернега 12 мест/обязанности/3 edu, Касаев,
  Мездриков, Бариева) и портфолио (Настенко, Бадзыма, Упоров).

**Мульти-роль в одной компании (edge-case):** когда в резюме несколько должностей
внутри ОДНОЙ компании с внутренними периодами (напр. «ООО Фрезениус Каби»: Бухгалтер
→ Менеджер по проектам → Главный бухгалтер, даты через дефис `-`), парсер создаёт
отдельные записи опыта, где company/position могут съезжать (должность попадает в
company). Обязанности при этом сохраняются. Правильная модель «одна компания — много
ролей» — крупная переработка; отложено. PDF-превью покрывает просмотр.

**Навыки без разделителей:** если hh выводит теги через одиночные пробелы в одной
строке («SQL Python Управление проектами»), они сохраняются слитной строкой (не
дробятся, т.к. навыки многословны). Данные не теряются; при поиске можно токенизировать.

**Известные ограничения rule-based (edge-cases, данные НЕ теряются):**
- hh: 2-3 позиции из 12 без должностного маркера → position пустой (обязанности
  на месте). Редко «сфера деятельности» просачивается в position.
- Портфолио: `company` нестабильна на части блоков (голова блока вариативна —
  колонтитул ФИО / фрагмент сферы попадает в company). Должность/обязанности/даты
  корректны. Полировка регулярок даёт diminishing returns и риск регрессий на hh.
- Английские/нарративные CV (Вольпе) → null → LLM (rule-based неприменим).
- Возможное развитие: гибрид «rule-based структура + точечный LLM на пустые поля».

**Качество PDF-парсинга** (при необходимости):
